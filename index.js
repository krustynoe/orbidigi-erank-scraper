// index.js — eRank scraper con login Sanctum + JSON + fallback HTML estable
...
function normalizeCountry(v) {
  ...
  const map = {
    "usa": "USA", "us": "USA", "united states": "USA", "eeuu": "USA",
  ...
  return map[s] || (s ? s.toUpperCase() : "USA");
}
function normalizeSource(v) {
  const s = String(v || "").trim().toLowerCase();
  // eRank espera 'source' (etsy, amazon, ebay...). Por defecto etsy.
  if (!s) return "etsy";
  if (["etsy","amazon","ebay","walmart","aliexpress"].includes(s)) return s;
  return "etsy";
}
...
async function callErank(pathname, query = {}) {
  ...
  // Intento directo con cookies (request.newContext + storageState)
  ...
  if (resp.ok() && ct.includes("application/json")) {
    return { url, json: JSON.parse(text), html: null };
  }
  ...
  if (out.ok && out.type.includes("application/json")) {
    ...
    catch { return { url, json: null, html: out.text }; }
  }
  return { url, json: null, html: out.text || text };
}
...
app.get("/erank/keywords", async (req, res) => {
  try {
    ...
    const country = normalizeCountry(req.query.country || "USA");
    const source = normalizeSource(req.query.marketplace || "etsy");
