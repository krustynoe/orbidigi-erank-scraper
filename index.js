// index.js — eRank JSON via Playwright (Sanctum login) + page.fetch fallback; sin page.fill
// Requiere package.json deps: express, cheerio, "playwright": "1.56.1"
...
let browser, ctx;          // Playwright Browser / BrowserContext
let lastLoginAt = 0;

// ---- 1) Login a Sanctum por API, no por teclado ----
async function ensureContextLogged(force = false) {
  const fresh = Date.now() - last > 0 && Date.now() - lastLoginAt < 20*60*1000;
  ...
  const login = await rq.post("https://members.erank.com/login", {
    ...
    headers: { "X-Requested-With": "XMLHttpRequest", "X-XSRF-TOKEN": decodeURIComponent(xsrf), "User-Agent": UA }
  });
...
// ---- 2) Lector JSON por la misma sesión (primero request, luego fallback por page.fetch) ----
async function callErankJson(pathname, query = {}) {
  ...
  // Cárgate la cookie de sesión en el client de request
  ...
  // fallback: usar page.fetch con cabeceras + cookies de la sesión viva
  const page = await (await ensureContextLogged()).newPage();
  await page.goto("https://members.erank.com/keyword-review", { waitUntil: "domcontentloaded", timeout: 120000 });
...
app.get("/erank/keywords", async (req,res)=>{
  try {
    ...
    if (!results.length) {
      ...
      const page = await (await ensureContextLogged()).newPage();
      await page.goto(`https://members.erank.com/keyword-tool?country=${encodeURIComponent(country)}&source=${encodeURIComponent(marketplace)}&keyword=${encodeURIComponent(q)}`, { waitUntil: "networkidle", timeout: 180000 });
