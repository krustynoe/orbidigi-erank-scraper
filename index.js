...
async function fetchRenderedHtml(url, waitForSel = 'body') {
  const params = {
    ...
    url,
    js_render: 'true',
    // many WAFs require this to be set to forward our Cookie correctly:
    custom_headers: 'true',
    // speed up a bit but keep JS working; DO NOT use "images" (invalid)
...
app.get('/erank/debug', async (req, res) => {
  try {
    ...
    const html = await fetchRenderedHtml(target, '.trend-card, [data-testid="trend"], h2, h3');
...
app.get('/erank/keywords', async (req, res) => {
  try {
    ...
    const html = await fetchRenderedHtml(TREND_URL, '.trend-card, [data-testid="trend"], h2, h3');

    const $ = cheerio.load(html);
    const words = new Set();

    // Try multiple selectors for titles/keywords
    $('.trend-card .title, [data-testid="trend-title"], .trend-title, h2, h3').each((_, el) => {
      const t = $(el).text().trim();
      if (t) words.add(t);
    });

    // Some pages list keywords as chips/badges
...
app.get('/erank/research', async (req, res) => {
  try {
    const q = String(req.query.q || '').toLowerCase();
    const html = await fetchRenderedHtml(TREND_URL, '.trend-card, [data-testid="trend-card"]');

    const $ = cheerio.s3 ? cheerio.s3 : cheerio; // safe
    const items = [];
...
// --- Etsy (público) via ZenRows + Cheerio (sin cookies CSRF)
app.get('/erank/products', async (req, res) => {
  try {
    const q = String(req.query.q || '');
    const url = `https://www.etsy.com/search?q=${encodeURIComponent(q)}`;
    const html = await fetchRenderedHtml(url, 'li[data-search-result], .v2-listing-card');
    const $ = cheerio.load(html);
    const items = [];
...
app.get('/erank/mylistings', async (req, res) => {
  try {
    ...
    const html = await fetchRenderedHtml(url, '.wt-grid__item-xs-6, .v2-listing-card');
    const $ = cheerio.load(html);
    const items = [];
